...
public class Hangman {
	...
	public boolean targetContains(String target, char letter) {
		...
		//if the target word contains the letter then return true, else false
	...
	public boolean hasFoundTarget(String target, String guess) {
		...
		// if the target word and the guess are equal then return true, else false
		boolean guessCheck;
	...
	public String getCurrentGuessForDisplay(String currentGuess) {
		...
		String currentNoSpaces, currentGuessEnd;
		currentNoSpaces = currentGuess.replace('*', '_'); //replace all *'s with _'s
		StringBuilder currentSpaces = new StringBuilder(); //make a new stringbuilder
		if(currentNoSpaces.length() > 0)
		{
			currentSpaces.append(currentNoSpaces.charAt(0)); //append the first character into the stringbuilder
			for(int i = 1; i < currentNoSpaces.length(); i++)
			{
				currentSpaces.append(" "); //append a space after the previous character
				currentSpaces.append(currentNoSpaces.charAt(i)); //append the next character of the word
			}
		}
		currentGuessEnd = currentSpaces.toString(); //converts stringbuilder into string
		
		return currentGuessEnd;
	}
	...
	public String updateCurrentGuess(String target, String guess, char letter) {
		...
		String updatedGuess,guessedLetter;
		StringBuilder targetSB = new StringBuilder(); //made new stringbuilder
		StringBuilder guessSB = new StringBuilder(); //made new stringbuilder
		targetSB.append(target); // stringbuilder contains target word
		guessSB.append(guess); //stringbuilder contains guess word
		guessedLetter = String.valueOf(letter); //letter turned into string to allow use in stringbuilder
		int i = targetSB.indexOf(guessedLetter); //finds first instance of letter in target
		while(i >= 0)
		{
			guessSB.setCharAt(i,letter); //replaces the instance of the guess character into the guessed word stringbuilder
			i = targetSB.indexOf(guessedLetter,i+1); //i changes to find the next instance of that letter in the target word
		}
		updatedGuess = guessSB.toString(); //converts stringbuilder to string
